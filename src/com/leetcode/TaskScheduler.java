...
public class TaskScheduler {
    ...
    public static void main(String[] args) {
       /* int result  = new TaskScheduler().leastInterval(new char[]{'A', 'A', 'B', 'B', 'C', 'C', 'D', 'D', 'E', 'E', 'F', 'F', 'G', 'G', 'H', 'H',
                        'I', 'I', 'J', 'J', 'K', 'K', 'L', 'L', 'M', 'M', 'N', 'N', 'O', 'O', 'P', 'P', 'Q', 'Q', 'R', 'R', 'S', 'S', 'T', 'T', 'U',
                        'U', 'V', 'V', 'W', 'W', 'X', 'X', 'Y', 'Y', 'Z', 'Z'}
                , 2);
        System.out.println("result = " + result);*/
        int result = new TaskScheduler().leastInterval(new char[]{'A', 'A', 'A', 'B', 'B', 'B'}, 2);
        System.out.println("result = " + result);

       /* int result = new TaskScheduler().leastInterval(new char[]{
    ...
    public int leastInterval(char[] tasks, int n) {
        ...
        TreeMap<Character, Integer> mapOfCharacterOccurrence = new TreeMap<>();

        for (int i = 0; i < tasks.length; i++) {
            if (mapOfCharacterOccurrence.containsKey(tasks[i])) {
                mapOfCharacterOccurrence.put(tasks[i], mapOfCharacterOccurrence.get(tasks[i]) + 1);
            } else {
                mapOfCharacterOccurrence.put(tasks[i], 1);
            }
        }

        Map<Character, Integer> sortedByValueDesc = mapOfCharacterOccurrence.entrySet()
                .stream()
        ...
        // minus 1 as the elements after A__A__A idle time should be 2*2 as after last most frequent element there is no idle time
        Map.Entry<Character, Integer> mostFrequentChar = sortedByValueDesc.entrySet().stream().findFirst().get();
        int mostFrequent = mostFrequentChar.getValue() - 1;
        //end
        int idleTime = mostFrequent * n;
        sortedByValueDesc.remove(mostFrequentChar.getKey());


        for (Map.Entry<Character, Integer> entry : sortedByValueDesc.entrySet()) {
            int numberOfOccurrence = entry.getValue();
            idleTime -= Math.min(mostFrequent, numberOfOccurrence);
